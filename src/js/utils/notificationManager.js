...
class NotificationManager {
    ...
    async notify(message, type) {
        ...
        this.#queue.push({ message, type, COOLDOWN_MS });
        this.#processQueue();
    }
    ...
    async #processQueue() {
        ...
        this.#isActive = true;
        const { message, type, COOLDOWN_MS } = this.#queue.shift();

        await this.#showNotification(message, type, COOLDOWN_MS);

        this.#isActive = false;
        this.#processQueue();
    }

    #showNotification(message, type, COOLDOWN_MS) {
        return new Promise((resolve) => {
            ...
            const notification = `
                <div class="notification-container ${type} rounded-normal" data-notification-id="${notificationId}" role="alert">
                    ${NotificationManager.icons[type] || NotificationManager.icons.info}
